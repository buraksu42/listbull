...
/**
 * All lists the user can see — joined via list_members (every owner gets a
 * row at create-time, so this covers owned + shared lists in one query).
 * Inbox first, then by createdAt asc.
 */
export async function listListsForUser(userId: string): Promise<List[]> {
  const rows = await db
    .select({ list: lists })
    .from(listMembers)
